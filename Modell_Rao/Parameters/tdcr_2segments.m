...
n = [6;0]; %number of spacerdisks on the 2 sections, n(1) corresponds to proximal section
n_disk=sum(n); %number of spacerdisks
L = [6*0.02;0.001]; % Lenght of the section in m
l=[L(1)/n(1)*ones(1,n(1)) L(2)/n(2)*ones(1,n(2))]; %array of segment lengths of each subsegment

% Tendon position on the disks
r_disk=8*1e-3; %distance of routing channels to backbone-center[m]
