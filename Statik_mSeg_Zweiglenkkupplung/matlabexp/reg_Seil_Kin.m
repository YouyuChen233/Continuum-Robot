clc;close all;
% getGleichungssystem
filenameDGL=sprintf('CalcFun\\DGL_mDisks_Seil.m');
fileID=fopen(filenameDGL,'w');
fprintf(fileID,sprintf('function unknown = DGL_mDisks_Seil(y,pars)\n'));
...
for i=1:pars.nseil
    if abs(pars.diffL(i))>0
        fprintf(fileID,'unknown(%d,1)=sum(getDelta_l%d(y,pars))-pars.diffL(%d);\n',pars.ndsk+ii,i,i);
        ii=ii+1;
    end
end
fprintf(fileID,'end');
...
clear ans dydt1 expression fileID filename filenameDGL filenameI i ii...
    replace str tmp*;
delete *.stats 
