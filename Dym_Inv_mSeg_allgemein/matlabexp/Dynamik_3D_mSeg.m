...
%Solver
odefun = @(t,y)([y(2*pars.ndsk+1:pars.ndsk*4);-Massen(y,pars)\DGL_mDisks_Inv_atan(y,pars)]);%;zeros(5,1)]);
[t,y]=ode45(odefun, tspan, y0);
%q-Parametisierung nach alpha-Parametisierung umrechnen
phi=zeros(size(y,1),pars.ndsk);
kappa=zeros(size(y,1),pars.ndsk);
for i=1:size(y,1)
    for ii=1:pars.ndsk
...
for i=1:size(y,1)    
    ...
    for ii=1:pars.ndsk
        E_p(i)=E_p(i)+0.5*pars.E*pars.Jxx*(kappa(i,ii)^2)*pars.L0;
    end
